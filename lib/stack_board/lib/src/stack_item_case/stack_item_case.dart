...
import 'package:cardmaker/stack_board/lib/src/stack_item_case/config_builder.dart';
...
class StackItemCase extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _StackItemCaseState();
}
...
class _StackItemCaseState extends State<StackItemCase> {
  ...
  Size _calculateNewSize(
    ...
    final double originalDistance = _caculateDistance(
      startGlobalPoint,
      startOffset,
    );
    final double newDistance = _caculateDistance(
      dud.globalPosition,
      startOffset,
    );
  ...
  Widget _content(BuildContext context, StackItem<StackItemContent> item) {
    ...
    final Widget? content = widget.childBuilder?.call(item);
      ...
      if (textContent.showBackground == true) {
        if (textContent.showStroke == true) {
          backgroundPaint
            ...
            ..strokeWidth = textContent.strokeWidth ?? 0.0;
        }
      } else {
